...
int run_workers(vector<wordindex>& files, vector<string> filenames,
  ...
  for(int i = 0; i < num_threads;i++){
    wordindex file;
    file.filename = filenames[start_pos + i];
    files.push_back(file);
    threads.push_back(std::thread(find_word,&files[start_pos + i],word));
    //printf("Launched %d-th thread in group\n",i);
  }
  ...
  for(int i = 0; i < num_threads; i++){
    threads[i].join();
    //printf("Joined %d-th thread in group\n",i);
    sum += files[start_pos + i].count;
  }
