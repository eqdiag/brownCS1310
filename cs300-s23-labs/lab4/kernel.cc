...
void copy_mappings(x86_64_pagetable* dst, x86_64_pagetable* src){
    ...
    //Assume dst has no mappings

    //Iterate through all src pages and get their phys addresses
    //Use these to construct 
    for(vmiter it(src,0); it.va() < MEMSIZE_VIRTUAL;it += PAGESIZE){
        //If mapped address, then copy it
        if(it.pa() != (uintptr_t) -1){
            vmiter(dst,it.va()).map(it.pa(),it.perm());
            log_printf("VA %p maps to PA %p with PERMS %p %p %p\n",
            it.va(),it.pa(),PTE_P & it.perm(),PTE_W & it.perm(),PTE_U & it.perm());
        }
    }
...
void kernel(const char* command) {
    ...
    //Copy kernel_pagetable mapping
    copy_mappings(kernel_pagetable_copy,kernel_pagetable);

    //Launch allocator user program
