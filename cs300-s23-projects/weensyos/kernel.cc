...
void kfree(void* kptr) {
    ...
    if(!pages[(uintptr_t) kptr / PAGESIZE].used()) return;

    //Only needs to reduce count in case multiple processes reference this
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    for(vmiter it(kernel_pagetable); it.va() < PROC_START_ADDR; it += PAGESIZE){
        //Just map kernel space to process
        vmiter pit(ptable[pid].pagetable,it.va());
        pit.map(it.pa(),it.perm());
    }
    ...
    for (loader.reset(); loader.present(); ++loader) {
        ...
        for (uintptr_t a = round_down(loader.va(), PAGESIZE);
            ...
            if(loader.writable()){
                ...
                it.map((uintptr_t)pa,PTE_P | PTE_U);
            }
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    //In valid user memory region
    if(addr < PROC_START_ADDR || addr >= MEMSIZE_VIRTUAL) return -1;
    
    void * pa;
    if((pa = kalloc(PAGESIZE)) == nullptr) return -1;
    //Update process virtual memory
    if( vmiter(current,addr).try_map(pa, PTE_P | PTE_W| PTE_U) == -1){
        //If virtual mapping fails, need to free up physical page
        kfree(pa);
        return -1;
    };
...
void free_proc(proc * p){
  //First free all non-page table pages in user space
  for(vmiter it(p,PROC_START_ADDR);it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE){
    if(it.user()){
        kfree((void*)it.pa());
    }
  }
...
pid_t syscall_fork() {
    ...
    //Any time a failure occurs in this loop (after PROC_START_ADDR) 
    //we must free all kalloced memory in new process pagetable

    //Copy parent process vm mappings but allocate new pages
    for(vmiter it(current);it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE){
        //Simply copy kernel space
        if(it.va() < PROC_START_ADDR){
            if( vmiter(child_pt,it.va()).try_map(it.pa(),it.perm()) == -1) return -1;
        }else{
            //Only copy user pages
            if(it.user()){

                //If writeable page, then create a copy
                if(it.writable()){
                    void * pa = kalloc(PAGESIZE);
                    if(!pa){
                        free_proc(&ptable[pid]);
                        return -1;
                    }
                    //Copy data from physical page of parent
                    memcpy(pa,(const void*)it.pa(),PAGESIZE);
                    if(vmiter(child_pt,it.va()).try_map(pa,it.perm()) == -1){
                        free_proc(&ptable[pid]);
                        return -1;
                    }
                }else{ //If readable, then don't allocate a new physical page, just inc ref count to phys page
                    if(vmiter(child_pt,it.va()).try_map(it.pa(),it.perm()) == -1){
                        free_proc(&ptable[pid]);
                        return -1;
                    }
    ...
    //Fill in process data in ptable
    ptable[pid].pid = pid;
    ptable[pid].state = P_RUNNABLE;
    ptable[pid].regs = current->regs;

    //Set respective return values
